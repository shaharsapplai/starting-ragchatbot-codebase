...
class TestConfiguration:
    ...
    def test_anthropic_api_key_is_set(self):
        ...
        assert (
            len(api_key) > 10
        ), f"ANTHROPIC_API_KEY looks invalid (length: {len(api_key)})"
        # Most Anthropic keys start with 'sk-ant-'
        if not api_key.startswith("sk-ant-"):
            pytest.warns(
                UserWarning, "API key doesn't start with 'sk-ant-' - may be invalid"
            )
    ...
    def test_chroma_path_exists_or_creatable(self):
        ...
        if os.path.exists(chroma_path):
            assert os.path.isdir(
                chroma_path
            ), f"{chroma_path} exists but is not a directory"
        else:
...
class TestDocumentsLoaded:
    ...
    def test_vector_store_has_content(self, loaded_vector_store):
        ...
        assert (
            course_count > 0
        ), "No courses in vector store - documents may not be loaded"
...
class TestAnthropicAPIConnection:
    ...
    def test_anthropic_api_reachable(self):
        ...
        try:
            ...
            pytest.fail(
                f"Unexpected error connecting to Anthropic: {type(e).__name__}: {e}"
            )
